...
int rsa_init_public_key(
    ...
    char* line = fgets(key_buffer, BUFFER_LEN, input_key);
    if(!line) {
        fprintf(stderr, "Failed to read first number from public key file.\n");
        fclose(input_key);
        return 1;
    }
    ...
    line = fgets(key_buffer, BUFFER_LEN, input_key);
    if(!line) {
        fprintf(stderr, "Failed to read second number from public key file.\n");
        fclose(input_key);
        return 1;
    }
...
int rsa_write_private_key(
    ...
    size_t ascii_size = mpz_sizeinbase(key->n, 10) + 2;
...
int rsa_write_public_key(
    ...
    size_t written = fwrite(key_buffer, 1, ascii_size, output_key);
    if(written != ascii_size) {
        fprintf(stderr, "Failed to write first line of public key file.\n");
        fclose(output_key);
        return 1;
    }
    fputs("\n", output_key);
    ...
    written = fwrite(key_buffer, 1, ascii_size, output_key);
    if(written != ascii_size) {
        fprintf(stderr, "Failed to write second line of public key file.\n");
        fclose(output_key);
        return 1;
    }
    fputs("\n", output_key);
